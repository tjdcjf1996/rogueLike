...
async function displayStatus(stage, player, monster) {
  ...
  console.log(
    chalk.cyanBright((' ').repeat(44)+`| Stage: ${stage} |`,'\n') +
    chalk.blueBright(
...
//---------------------------------------------------------------------------------------------------------------------------- battle 함수 선언
const battle = async (stage, player, monster, bonusNum) => {
  ...
  while(player.hp > 0 && monster.hp > 0) {
    console.clear();
    // 스테이지 5판당 서비스 스탯 제공
    if(bonusNum == 1 && stage !== 1 && (stage-1) % 2 === 0){
    ...
    console.log(
      chalk.green(
        `\n`+ ' '.repeat(15) +`| 1. 공격한다 | 2. 연속공격(10% 확률) | 3.기도메타(5% 확률 / 잔여:${prey}회) | \n\n`,
      ),
    );
...
export async function startGame() {
  ...
  // 스테이지 시작할 때마다 Monster 객체 생성
    ...
    await battle(stage, player, monster, bonusNum);
